...
def load_scenarios(scenario_path, seed_path):
    ...
    for scenario, group in scenarios.groupby("scenario"):
        combined = pd.concat(
            [group[["entity", "onset_year"]], unaffected[["entity", "onset_year"]]],
            ignore_index=True,
        )
        rows.append((scenario, combined))
    return rows
...
def build_outputs(base_dir=".", output_dir="results", start_year=1945, end_year=2025):
    ...
    affected = diagnostic.loc[
        diagnostic["four_nation_classification_changed"],
        ["scenario", "year", "genuine_count", "baseline_genuine_count", "count_delta_from_baseline"],
    ]
